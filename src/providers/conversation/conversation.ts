...
return new Promise(resolve => {  this.http.post('http://localhost:3333/sendmessage', messages).subscribe(data => {
...
get(dets) {
  ...
  return new Promise(resolve => {  this.http.post('http://localhost:3333/getmsg', dets).subscribe(data => {
      if(data){
